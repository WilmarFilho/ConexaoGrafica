...
<aside class="lateral" aria-label="Conteúdo complementar">
    ...
    <section class="lateral__bloco">
        ...
        <form class="busca-lateral" role="search" method="get" action="<?php echo esc_url(home_url('/')); ?>">
            ...
            <input type="search" id="busca-blog" name="s" value="<?php echo esc_attr(get_search_query()); ?>" placeholder="Procure por post...">
    ...
    <?php if (file_exists($banner)) : ?>
        <a class="lateral__banner" href="<?php echo esc_url(home_url('/publique-conosco/')); ?>">
            <img src="<?php echo esc_url(get_template_directory_uri().'/assets/img/banner-lateral.png'); ?>"
                 alt="Publique conosco: transforme seu manuscrito em livro" loading="lazy">
        </a>
    <?php endif; ?>
        ...
        <section class="lateral__bloco">
            ...
            <p class="nuvem">
                <?php foreach ($tags as $tag) : ?>
                    <a href="<?php echo esc_url(get_tag_link($tag)); ?>" style="font-size: <?php echo esc_attr(min(19, 13 + $tag->count)); ?>px"><?php echo esc_html($tag->name); ?></a>
                <?php endforeach; ?>
            </p>
        </section>
    <?php endif; ?>
